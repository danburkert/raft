...
impl<S, M> Server<S, M> where S: Store, M: StateMachine {
    ...
    fn replica_mut(&mut self) -> &mut Replica<S, M> {
        &mut self.replica
    }
...
impl<S, M> Handler for Server<S, M> where S: Store, M: StateMachine {
    ...
    fn readable(&mut self, reactor: &mut EventLoop<Server<S, M>>, token: Token, _hint: ReadHint) {
        ...
        match token {
            ...
            token => {
                self.connections[token].readable(reactor, self).unwrap();
            }
            _ => unreachable!()
        }
    }
...
impl Connection {
    ...
    where S: Store, M: StateMachine {
        ...
        if self.write_queue.is_empty() {
            ...
            Ok(event_loop.reregister(&self.stream, self.token, self.interest, poll_opt()));
        }
        Ok(())
    }
    ...
    where S: Store, M: StateMachine {
        ...
        match emit_type {
            EmitType::None => (),
            EmitType::Reply => {
                self.send_message(event_loop, Rc::new(outgoing_message_builder)).unwrap()
            },
            EmitType::Broadcast => {
                server.broadcast(event_loop, outgoing_message_builder).unwrap()
            },
        }
    }
}
